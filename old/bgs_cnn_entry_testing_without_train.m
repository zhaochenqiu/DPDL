...
templist = sum(matrix_fg_bk,2);
pos_sq = find(templist ~= 0);

if frames_train_sq > 0
    [row_t column_t] = size(pos_sq);

    index = linspace(1,max(size(pos_sq)),min([frames_train_sq max(size(pos_sq))]));
%    index = linspace(1,max(size(pos_sq)),min([frames_train_sq max(size(pos_sq))]) + 2);
%    index = index(2:end - 1);
    ...
    train_frames = [train_frames pos_sq'];

    for i = 1:len
%        imdb_sq = getImdb_test(im_pa,im_ft,tr_pa,tr_ft,pos_sq(i),size_block,rag_block,'train');

%        imdb_sqset = {imdb_sqset{:}, imdb_sq};
    end
end

%imdb = mixImdb(imdb_fgset{:},imdb_bkset{:},imdb_sqset{:});
% imdb = equImdb_small(imdb);
%oldtrainimdb = imdb;
...
% cnn_mnist_experiments_self(imdb,path_save);




net     = load([path_save '/'  sprintf('net-epoch-%i.mat',g_epoch)] );
net     = net.net;
...
filename = 'train_frames.txt';
txtWrite(sort(unique(train_frames)),[sv_pa filename]);
