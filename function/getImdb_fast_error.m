...
im = createData_fast(block_im,round(row_im/2),round(column_im/2),len);
...
% mark = 1 train
% mark = 0 test
% getImdb_fast_entry(block_im, data, labimg, curimg, im,  [row_bl column_bl byte_bl frames_bl len row_im column_im byte_im round(sqrt(frames_bl))], mark);


data = reshape(block_im,row_im,column_im,byte_im,row_bl*column_bl);
labs = reshape(labimg,1,column_bl*row_bl);
%{
for posr = 1 + len:row_bl - len
    for posc = 1 + len:column_bl - len
        im = createData_fast(block_im,posr,posc,len);
        lb = labimg(posr,posc);
...
[row_im column_im byte_im] = size(srcim);
