...
labimg = double(imread_small([tr_pa '/' tr_fs{curindex}]));
curimg = double(imread_small([im_pa '/' im_fs{curindex}]));
...
for i = 1:max(size(index))
    ...
    im = double(imread_small([im_pa '/' im_fs{index(i)}]));
%    im = im - curimg;
